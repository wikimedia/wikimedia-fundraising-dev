...
# Check for $1 xdebug ini file, and writes default if it doesn't exist.
# (Default file is assumed to exist at /tmp/default-xdebug.ini.)
# Then, see if the port set in the xdebug file is the one requested. If not, update it.
# (Also assumes $xdebug_port has been set already.)
create_or_maybe_update_xdebug_ini () {
	if [[ ! -e $1 ]]; then
		echo "Creating $1."
		cp /tmp/default-xdebug.ini $1
		echo

	else
		port_from_file=$(sed -n 's/.*xdebug.remote_port *= *\([^ ]*.*\)/\1/p' < $1)
		if [[ $port_from_file != $xdebug_port ]]; then
			echo "Existing $1 sets a different port for xdebug; updating and backing up."
			backup $1
			sed -i -e "s/.*xdebug.remote_port=.*/xdebug.remote_port=$xdebug_port/" $1
		else
			echo "$1 exists and does not need updating."
		fi
	fi
}
...
cat << EOF > /tmp/default-xdebug.ini
...
do
	create_or_maybe_update_xdebug_ini $i
done
