...
CIVICRM_BACKUP_PATH="./.backup/sql/wmfcivi.sql"
TEST_BACKUP_PATH="./.backup/sql/wmftest.sql"
...
docker compose exec -T $CIVICRM_SERVICE_NAME mysql -uroot -h$DATABASE_SERVICE_NAME wmfcivi < $CIVICRM_BACKUP_PATH
echo "Restoring standalone Test database..."
docker compose exec -T $CIVICRM_SERVICE_NAME mysql -uroot -h$DATABASE_SERVICE_NAME wmftest < $TEST_BACKUP_PATH
echo
