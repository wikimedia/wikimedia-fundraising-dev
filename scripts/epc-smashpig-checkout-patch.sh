...
if ! docker compose exec email-pref-ctr bash -c '[ -d "/var/www/html/vendor/wikimedia/smash-pig/.git" ]'; then
  echo "*** Backing up existing SmashPig library..."

  # Check if the backup directory exists and remove it
  docker compose exec email-pref-ctr bash -c 'if [ -d "/var/www/html/vendor/wikimedia/smash-pig-backup" ]; then rm -rf /var/www/html/vendor/wikimedia/smash-pig-backup; fi'

  # Backup smash-pig directory
  docker compose exec -w "/var/www/html/" email-pref-ctr mv vendor/wikimedia/smash-pig/ vendor/wikimedia/smash-pig-backup
  ...
  docker compose exec email-pref-ctr git clone "https://gerrit.wikimedia.org/r/wikimedia/fundraising/SmashPig" /var/www/html/vendor/wikimedia/smash-pig
fi
docker compose exec -w "/var/www/html/vendor/wikimedia/smash-pig" email-pref-ctr git checkout master

echo "*** Fetching patchset/revision $PATCH_ID/$LATEST_REVISION"
# Delete branch if it already exists to get latest changes
docker compose exec -w "/var/www/html/vendor/wikimedia/smash-pig" email-pref-ctr git branch -D change-$PATCH_ID &>/dev/null || :
# Checkout the latest patchset
docker compose exec -w "/var/www/html/vendor/wikimedia/smash-pig" email-pref-ctr git fetch https://gerrit.wikimedia.org/r/wikimedia/fundraising/SmashPig refs/changes/$LAST_TWO_DIGITS/$PATCH_ID/$LATEST_REVISION && \
docker compose exec -w "/var/www/html/vendor/wikimedia/smash-pig" email-pref-ctr git checkout -b change-$PATCH_ID FETCH_HEAD
