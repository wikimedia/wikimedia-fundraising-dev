...
docker compose exec -w "/srv/civi-sites/wmff/drupal" civicrm drush @wmff -vv --in=json cvapi Omnirecipient.load
#process from civicrm_mailing_provider_data to create unsubscribe activities.
echo -e "${BLUE}Running ${GREEN}Omnirecipient unsubscribe${BLUE}${NC}"
docker compose exec -w "/srv/civi-sites/wmff/drupal" civicrm drush @wmff -vv cvapi Omnirecipient.process_unsubscribes mail_provider=Silverpop rowCount=5000
#Manage omnimail job to issue deletion requests for Silverpop.
echo -e "${BLUE}Running ${GREEN}Omnirecipient forgetme${BLUE}${NC}"
docker compose exec -w "/srv/civi-sites/wmff/drupal" civicrm drush @wmff -vv cvapi Omnirecipient.process_forgetme mail_provider=Silverpop
...
docker compose exec -w "/srv/civi-sites/wmff/drupal" civicrm drush @wmff -vv cvapi Omnirecipient.process_onhold mail_provider=Silverpop rowCount=5000
...
docker compose exec -w "/srv/civi-sites/wmff/drupal" civicrm drush @wmff -vv cvapi Omnimailing.load mail_provider=Silverpop
#Get group member data from Silverpop in the 'no cid' group
echo -e "${BLUE}Running ${GREEN}Omnigroupmember load${BLUE}${NC}"
docker compose exec -w "/srv/civi-sites/wmff/drupal" civicrm drush @wmff -vv --in=json cvapi Omnigroupmember.load
...
docker compose exec -w "/srv/civi-sites/wmff/drupal" civicrm drush @wmff cvapi omnigroupmember.match group_id=310 limit=10000
