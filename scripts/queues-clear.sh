...
current_seq=$("${script_dir}/queues-cli.sh" GET sequence_contribution-tracking | tr -d '"\n\r')
echo ${current_seq}

# Clear everything
echo "FLUSHALL"
"${script_dir}/queues-cli.sh" FLUSHALL

if [ "${current_seq}" != "(nil)" ]; then
  # Restore sequence number
  echo "SET sequence_contribution-tracking ${current_seq}"
  "${script_dir}/queues-cli.sh" SET sequence_contribution-tracking "${current_seq}"
fi
